...
class DNS_SiamCARTracker(SiamCARTracker):
    def __init__(self, model, denoiser, cfg):
...
class DNS_SiamCARPipeline():
    def __init__(self, args, denoiser):
        super(DNS_SiamCARPipeline, self).__init__()
        if not args.config:
            args.config = './experiments/SiamCAR/config.yaml'
        if not args.snapshot:
            args.snapshot = './experiments/SiamCAR/model.pth'
        ...
        self.model = load_pretrain(self.model, args.snapshot).cuda().eval()
        self.denoiser = denoiser
        self.tracker = DNS_SiamCARTracker(self.model, self.denoiser, cfg.TRACK)
